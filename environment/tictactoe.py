...
class TicTacToeEnv(gym.Env):
    ...
    def __init__(self, alpha=0.02, show_number=False, opponent_mode='random'):
        self.action_space = spaces.Discrete(NUM_LOC)
        self.observation_space = spaces.Discrete(NUM_LOC)
        self.alpha = alpha
        self.opponent_mode = opponent_mode  # 对手模式 random:随机对手 human:人类对手
    ...
    def step(self, action):
        ...
        # place
        if self.board[loc] != 0:
            return self._get_obs(), -100, self.done, None  # 对于重子的位置给以很大的惩罚
        ...
        return self.opponent(reward)

    def opponent(self, reward):
        action = self.action_space.sample()
        if self.opponent_mode == 'random':
            while self.board[action] != 0:
                action = self.action_space.sample()  # 随机选取不重复的位置

        else:
            self.render()
            action = self.input_postition()
            while action == -1:
                action = self.input_postition()

        self.board[action] = tocode('X')  # 下X
        status = check_game_status(self.board)
        logging.debug("check_game_status board {} mark '{}'"
                      " status {}".format(self.board, self.mark, status))
        if status >= 0:
    ...
    def input_postition(self):
        """
        处理位置输入
        :return: -1:输入不合法 其他：合法输入位置
        """
        input_str = input("请输入你下X的位置（从左上角的0,0到右下角的2,2）: ")
        if len(input_str) > 3 or len(input_str) < 1:
            print("请输入正确的格式!")
            return -1
        y, x = int(input_str[0]), int(input_str[2])
